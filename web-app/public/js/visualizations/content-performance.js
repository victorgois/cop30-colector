...
function renderHashtagsVsEngagement(data) {
  ...
  section.append('p')
    ...
    .style('font-size', '14px')
    .style('margin-bottom', '15px')
    .text('Correlação entre quantidade de hashtags usadas e engajamento total');
  ...
  // Escalas
  const x = d3.scaleLinear()
    .domain([0, d3.max(data, d => d.hashtag_count)])
    .range([0, width]);

  const y = d3.scaleLog()
    .domain([1, d3.max(data, d => parseInt(d.total_engagement))])
    .range([height, 0])
    .nice();
  ...
  svg.append('g')
    ...
    .attr('fill', 'currentColor')
    .style('text-anchor', 'middle')
    .text('Engajamento Total (escala log)');
